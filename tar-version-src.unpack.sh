...
if test -e ./tar-version-src/${MY_TARVERSION}.tgz ; then
    rm -Rf ./tar-version-src/${MY_TARVERSION}
    bsdtar xzf ./tar-version-src/${MY_TARVERSION}.tgz -C ./tar-version-src/     
    exit 0;
fi; 
...
#gzcat ./tar-version-src/${MY_TARVERSION}.zdiff | patch -d ./tar-version-src/ -u 
