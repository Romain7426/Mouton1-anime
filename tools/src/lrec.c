...
static void print_content_of_the_dir(const char * dir_name);
...
int main(const int argc, const char * argv[]) {
  ...
  for (int i = 0; i < nb; i++) {
    print_content_of_the_dir(dirs_to_list[i]);
  }
...
void print_content_of_the_dir(const char * dir_name) {
  ...
  for (;;) {
    ...
    if ((ent->d_type & DT_DIR) != 0) {
      ...
      print_content_of_the_dir(new_name);
      continue;
    }
